package br.com.il.sistemasolar.controller;

import static org.junit.Assert.assertEquals;
import static org.mockito.ArgumentMatchers.anyLong;
import static org.mockito.Mockito.when;

import java.util.ArrayList;
import java.util.List;

import org.junit.Before;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.junit.runners.JUnit4;
import org.mockito.Mock;
import org.mockito.MockitoAnnotations;

import br.com.il.sistemasolar.model.Clima;
import br.com.il.sistemasolar.model.Resultado;
import br.com.il.sistemasolar.service.ClimaService;


@RunWith(JUnit4.class)
public class ClimaControllerTest {
	
	private ClimaController controller;
	
	private Clima clima;
	private List<Resultado> lstResultados;
	private List<Long> lstDias;
	
	@Mock
	private ClimaService climaService;

	@Before	
	public void before() throws Exception{
		
		MockitoAnnotations.initMocks(this);
		controller = new ClimaController();
		controller.setClimaService(climaService);
		
		clima = new Clima();
		clima.setClima("CHUVA");
		clima.setDia(1L);
		
		Resultado r = new Resultado();
		r.setClima("SECA");
		r.setDias(34L);
		
		lstResultados = new ArrayList<Resultado>();
		lstResultados.add(r);
		
		lstDias = new ArrayList<Long>();
		lstDias.add(5L);
		
		when(climaService.getPrevisao(anyLong())).thenReturn(clima);
		when(climaService.getResultado()).thenReturn(lstResultados);
		when(climaService.getDiaMaiorPeriodoDeChuva()).thenReturn(lstDias);
		
	}

	@Test
	public void testPrevisao() {
		
		assertEquals(clima, controller.getPrevisao(1L));
	}

	@Test
	public void testResultado() {
		
		assertEquals(lstResultados, controller.getResultado());
	}
	
	@Test
	public void testMaiorPeriodoDeChuva() {
		
		assertEquals(lstDias, controller.getDiaMaiorPeriodoDeChuva());
	}

}
